...
final class Module_Friends extends GDO_Module
{
	...
	public function renderTabs(): void
	{
		...
		$link3 = GDT_Link::make('link_incoming_friend_requests')->text('link_incoming_friend_requests', [$incoming])->href(href('Friends', 'Requests'));
		if ($incoming)
		{
			$link3->icon('alert');
		}
		$bar->addFields(
			...
			GDT_Link::make('link_friends')->text('link_friends', [$friends])->icon('group')->href(href('Friends', 'FriendList')),
			$link3,
